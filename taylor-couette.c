...
void calc_phi(void) {
    ...
    for (int i = 0; i <= Nx; i++)
        for (int j = 0; j <= Ny+1; j++) {
            r = sqrt((xf[i]-L/2)*(xf[i]-L/2) + (yc[j]-L/2)*(yc[j]-L/2));
            if (r <= ri) phi_u[i][j] = r - ri;
            else if (r >= ro) phi_u[i][j] = ro - r;
            else phi_u[i][j] = fmin(r - ri, ro - r);
        }
    for (int i = 0; i <= Nx+1; i++)
        for (int j = 0; j <= Ny; j++) {
            r = sqrt((xc[i]-L/2)*(xc[i]-L/2) + (yf[j]-L/2)*(yf[j]-L/2));
            if (r <= ri) phi_v[i][j] = r - ri;
            else if (r >= ro) phi_v[i][j] = ro - r;
            else phi_v[i][j] = fmin(r - ri, ro - r);
        }
}
    ...
    for (int i = 1; i <= Nx-1; i++)
        for (int j = 1; j <= Ny; j++)
            if (flag_u[i][j] == FORCE) {
                ...
                r = sqrt((xf[i]-L/2)*(xf[i]-L/2) + (yc[j]-L/2)*(yc[j]-L/2));
                theta = atan2(yc[j]-L/2, xf[i]-L/2);
                u_surf = r > (ri+ro)/2 ? 0 : omega*ri*sin(theta);
    ...
    for (int i = 1; i <= Nx; i++)
        for (int j = 1; j <= Ny-1; j++)
            if (flag_v[i][j] == FORCE) {
                ...
                /* Calculate the surface velocity. */
                r = sqrt((xc[i]-L/2)*(xc[i]-L/2) + (yf[j]-L/2)*(yf[j]-L/2));
                theta = atan2(yf[j]-L/2, xc[i]-L/2);
                v_surf = r > (ri+ro)/2 ? 0 : -omega*ri*cos(theta);
